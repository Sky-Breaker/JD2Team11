...
def _get_python_path(use_venv: bool) -> str:
    ...
    if (use_venv and venv_exists):
        ...
        if (os.name == "nt"):
            ...
            return str(project_root_path / '.venv' / 'Scripts' / 'python.exe')
        else:
            # Other system venv python path
            return str(project_root_path / '.venv' / 'bin'/ 'python')
    else:
        return 'python'

def run_command(args: list[str], capture_output=False, use_venv: bool=True):
    python = _get_python_path(use_venv)
